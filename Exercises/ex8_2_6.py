...
# Assuming errors is defined somewhere in your code
# Print the errors array to inspect its contents
print(errors)

# Check the shape of each element in the errors array
for i, error in enumerate(errors):
    print(f"Shape of element {i}: {np.shape(error)}")

# Ensure all elements are of the same shape before computing the mean
if all(np.shape(error) == np.shape(errors[0]) for error in errors):
    mean_error = np.mean(errors)
    print(f"Mean error: {mean_error}")
else:
    print("Errors array contains elements of different shapes.")
