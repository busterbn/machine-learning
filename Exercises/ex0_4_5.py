...
a6 = a1.copy()  # copy
a7 = a1  # alias
m1 = np.asmatrixrix("1 2 3; 4 5 6; 7 8 9")  # define matrix by string
m2 = np.asmatrix(a1.copy())  # copy array into matrix
m3 = np.asmatrix(np.array([1, 2, 3]))  # map array onto matrix
...
m = np.asmatrixrix("1 2 3; 4 5 6; 7 8 9")
m[0, 0]  # first element
m[-1, -1]  # last element
m[0, :]  # first row
m[:, 1]  # second column
...
m[-1, -1] = 10000
m[0:2, -1] = np.asmatrixrix("100; 1000")
m[:, 0] = 0
...
m1 = 10 * np.asmatrix(np.ones([3, 3]))
m2 = np.asmatrix(np.random.rand(3, 3))
